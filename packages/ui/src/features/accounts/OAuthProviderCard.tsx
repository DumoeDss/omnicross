...
type OAuthProviderId = 'claude' | 'codex' | 'gemini' | 'kimi' | 'grok' | 'copilot' | 'antigravity';
...
export function OAuthProviderCard({
  ...
  // codex + antigravity (loopback) and kimi/grok/copilot (device code) are the async + polled flows.
  const isAsyncOAuth =
    providerId === 'codex' || providerId === 'kimi' || providerId === 'grok' || providerId === 'copilot' || providerId === 'antigravity';
  const {
    busy,
    data,
    allowances,
  ...
  const cancelOAuth = async () => {
    ...
    else if (providerId === 'antigravity' && oauth) await cancelAntigravityOAuth(oauth.sessionId);
    ...
    <div className="space-y-4 rounded-lg border border-border bg-surface-1/50 p-4">
      ...
      {isAddMode && oauth && isAsyncOAuth ? (
        <CodexInlineSignIn
          ...
          sessionId={oauth.sessionId}
          onPoll={providerId === 'kimi' ? pollKimiOAuth : providerId === 'grok' ? pollGrokOAuth : providerId === 'copilot' ? pollCopilotOAuth : providerId === 'antigravity' ? pollAntigravityOAuth : pollCodexOAuth}
            ...
            providerId === 'kimi'
              ...
              : providerId === 'grok'
                ...
                : providerId === 'copilot'
                  ...
                  : providerId === 'antigravity'
                    ? t('accounts.antigravityOauth.description')
                    : undefined
          }
          onDone={() => {
            setOauth(null);
