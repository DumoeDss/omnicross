...
`omnicross` приймає вхідний LLM-запит — OpenAI `/v1/chat/completions`, Anthropic `/v1/messages`, Gemini тощо — визначає **який провайдер, обліковий запис і ключ** мають відповісти на нього (ваші власні API-ключі, пул із кількох ключів або підписка OAuth-ідентифікатора), пропускає його через конвеєр трансформатора й автентифікації, проксіює до хмарного провайдера — і повертає відповідь, перекодовану у формат дротового протоколу, що запросив викликач.
