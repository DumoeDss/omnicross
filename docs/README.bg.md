...
`omnicross` приема входяща LLM заявка — OpenAI `/v1/chat/completions`, Anthropic `/v1/messages`, Gemini и други — определя **кой доставчик, акаунт и ключ** трябва да я обработи (вашите собствени API ключове, пул от множество ключове или OAuth идентичност на абонамент), прекарва я през трансформаторен + удостоверителен конвейер и я проксира нагоре по веригата — повторно кодирайки отговора обратно в каквито и да е форматни изисквания на извикващия.
