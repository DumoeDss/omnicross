...
## 8. 2026-09-22：存量上游与订阅池的默认映射

原预配置只发生在新增 BYO provider 时，存量 provider 和订阅池没有默认表。
客户端密钥切换到上游绑定后，旧路由中已有的模型选择也没有进入新表。
现在 `resolveUpstreamModelMappings` 统一供目录 GET 和运行时路由组装使用：

- 已保存的表优先，包括显式空数组（用户选择透传）。
- 未保存表时，先继承启用的旧映射路由，保留目标模型和 effort。
- 再为缺少的通配行提供默认值：BYO 沿用提供商预配置规则，订阅池使用已知默认值；没有已知模型的池不凭空填充。
- 订阅池补充目录模型的同名映射，避免通配规则把用户明确选择的小模型替换为默认模型。
- 清空编辑器时保存 `[]`，配置归一化和旧路由迁移保留该选择，防止刷新后自动恢复默认表。

读取默认表不写配置；界面看到的有效表与运行时使用的表一致。既有手工表不被自动扩充或覆盖。
