...
**omnicross は、あなたの既存のサブスクリプションまたは API Key を使って、すべての AI アプリとコーディング CLI を一か所から動かせます。**

Claude Code、Codex、Gemini CLI — または OpenAI / Anthropic / Gemini の API 形式を話す任意のアプリ — を omnicross に向けるだけで、各リクエストをあなたが選んだプロバイダーとモデルにルーティングします。できること：

- **Claude / ChatGPT / Gemini のサブスクリプションログイン**でそのまま実行し、従量課金の API Key を省く；
- 複数の API Key をキープールとしてまとめ、自動ローテーションとフェイルオーバーを利用する；
- ある API 形式しか話せないツールから、別の形式を話すモデルを呼び出す — omnicross がリクエストとレスポンスをリアルタイムで変換する。

これらすべてをデスクトップアプリの GUI で管理できます — 設定ファイルを手動編集する必要はありません。
