...
`omnicross` принимает входящий LLM-запрос — OpenAI `/v1/chat/completions`, Anthropic `/v1/messages`, Gemini и другие — определяет, **какой провайдер, аккаунт и ключ** должен его обработать (ваши собственные API-ключи, пул из нескольких ключей или подписочная OAuth-идентичность), пропускает через конвейер трансформатора и аутентификации, проксирует запрос к вышестоящему провайдеру — и перекодирует ответ обратно в тот формат, который запросил вызывающий.
